...
The K-FIND fallback contract is defined for the future production chain but is not wired into `FoodSearchService` routing yet. Empty successful K-FIND search results are `no_results`, not provider errors, and may fall back to the next ordered provider. Known K-FIND provider failures are fallback eligible: `configuration`, `auth`, `permission`, `rate_limit`, `timeout`, `unavailable`, and `malformed_response`. Unexpected programming errors such as `TypeError`, `AttributeError`, `AssertionError`, or arbitrary `RuntimeError` must not be hidden by fallback and should continue through the existing error path. Fallback success is transparent to frontend responses; safe internal metadata can record provider, reason, operation, and safe upstream code for logging or metrics. `configuration`, `auth`, `permission`, and malformed upstream responses are operator alert candidates. Retry, cache, quota, and snapshot behavior are separate concerns.
