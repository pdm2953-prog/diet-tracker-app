...
| `fatsecret` | Uses K-FIND as the primary Korean production search provider and FatSecret as the ordered fallback. FatSecret credentials are still required; missing credentials are a configuration error and the app does not silently fall back to mock. |
...
## K-FIND Search And Diagnostics

K-FIND/MFDS food nutrition DB support is wired into production search when `FOOD_PROVIDER=fatsecret`. K-FIND is not a `FOOD_PROVIDER` option and is not selected by `create_food_provider`; `FoodSearchService` uses a separate ordered primary provider factory so the frontend remains provider-neutral.

K-FIND variables:

| Variable | Default | Notes |
| --- | --- | --- |
| `KFOOD_API_KEY_ENCODED` | none | Public data portal Encoding key value. Use it as the `serviceKey` query value as-is; do not URL encode it again. Never expose it to frontend, docs, tests, or logs. If missing while `FOOD_PROVIDER=fatsecret`, K-FIND treats the request as a configuration failure and FatSecret fallback remains eligible. |
...
When `FOOD_PROVIDER=fatsecret`, production search keeps verified `KoreanFoodCatalog` curated nutrition first, then tries K-FIND as the primary Korean search provider, and uses FatSecret only as the ordered fallback. The fallback path reuses the existing FatSecret catalog, provider-search, and alias safeguards instead of making K-FIND a frontend-visible provider option. Empty successful K-FIND search results are `no_results`, not provider errors, and may fall back to FatSecret. Known K-FIND provider failures are fallback eligible: `configuration`, `auth`, `permission`, `rate_limit`, `timeout`, `unavailable`, and `malformed_response`. Unexpected programming errors such as `TypeError`, `AttributeError`, `AssertionError`, or arbitrary `RuntimeError` are not hidden by fallback and continue through the existing error path. Fallback success is transparent to frontend responses; safe internal metadata can record provider, reason, operation, and safe upstream code for future logging or metrics. `configuration`, `auth`, `permission`, and malformed upstream responses are operator alert candidates. Retry, cache, quota, and snapshot behavior are separate concerns.
